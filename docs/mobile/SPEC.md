...
@media (max-width: 1023.98px) {
  :root {
    ...
    --m-lh-lead: 1.45;
    --m-lh-head: 1.1;
    --m-measure: 29em;                                         /* prose cap from 600 (~60 cpl measured; 34em ran 71 cpl, §17) */
...
- The nav gap is 16px; where the one-line wordmark would come closer than that to Menu (below ~383px) it stacks as EDCLOUD / VENTURE PARTNERS. With text enlarged the brand link clips (`overflow:hidden`) rather than running under Menu.
...
- Gutters are 32px and the content column is capped at `--m-measure` (29em) plus gutters and centred, so the white space is equal on both sides (§18).
...
- Conference-room on phones: `calc(100vw - 40px)`. On tablets: `min(31em, calc(100vw - 64px))` (the 493px column; `sizes` em is 16px).
...
3. No automatic hyphenation (`hyphens: manual`). A compound like "K-12" must not break at its hyphen (A-home-17). Wrap it in a `<span data-nowrap>` in the markup (`keepTogether()` in `components/KeepTogether.tsx`: "K-12", "long-term", dates, "Level AA"); the DOM text stays identical and qa:content unwraps the span. Below 1024 the span is an `inline-block` with `max-width:100%`: it moves to the next line whole, and still wraps inside itself if it is wider than a line at enlarged text sizes.
