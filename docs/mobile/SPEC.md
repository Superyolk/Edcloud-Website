...
@media (max-width: 1023.98px) {
  :root {
    ...
    --m-t-display: clamp(2rem, 1.35rem + 3.2vw, 3.25rem);      /* 32 / 34.1 / 46.2  Home H1, lh 1.06, 600, ls -0.01em */
    ...
    --m-lh-lead: 1.45;
    --m-lh-head: 1.1;
    --m-measure: 34em;                                         /* prose cap from 600 (~60 cpl) */
...
| `MQ_TABLET_WIDE` | `(min-width: 820px) and (max-width: 1023.98px)` | 820–1023: 3-up phases, real 4-column results table, partner portrait in a column |
...
- **Height:** `--nav-h` 56px plus `env(safe-area-inset-top)`. Sticky, with no hide-on-scroll. The transparent-to-solid change is a background-colour and `--m-elev-1` change only, with no height change, so CLS stays 0.
...
- **Fit:** 56 + 3×56 + 2×48 + foot ≈ 132 = 452px. At 844×390 the middle scrolls, so every link is reachable (C-cross-01, C-cross-02). The foot stays pinned.

### 5.4 SiteFooter (<1024)

- Black, `--m-section-top` padding, and 2 columns (`1.25fr 1fr`, 16px gap) at every phone width, including 320.
...
- **Markup:** `<h3 class={existing}><button type="button" aria-expanded aria-controls="{panel ids}" id="{btn id}">{existing heading text}</button></h3>`. The panel has `id`, `role="region"` and `aria-labelledby="{btn id}"`.
...
- **What and where:**
  ...
  - Placed at `right: max(16px, env(safe-area-inset-right))` and `bottom: calc(16px + env(safe-area-inset-bottom))`.
- **Visibility:**
  ...
  - It hides while `#contact` or the footer intersects the viewport.
...
| 07 | Contact Us | ~880 (E) | The rule header **first**, so the section boundary stays clear. Then the classroom photo as a 39:14 full-bleed band (`<picture>`, lazy, 125px tall at 390; A-home-12: it was 468px), then the H2 at `--m-t-accent`, then the form. First and last name sit 2-up (165px each; stacked under `MQ_XS`). Email, phone and message are full width. Labels are 14px/600 ink above the fields. Inputs are 48px tall with 16px text (no iOS zoom), a 1px `--slate` border (an existing token) and the 2px green `:focus-visible` ring instead of `outline:none` (C-cross-12). The textarea gets `rows={4}` with `min-height:120px`. SUBMIT is a full-width 52px ink pill, the last element, in the thumb zone. The `role="status"` line stays under it. Form attributes: §7.5. |
| — | Footer | ~412 | §5.4 |
...
- **ARIA** (added at all widths; invisible):
  ...
  - The panel gets `id="svc-panel" role="region" aria-labelledby={active button id}`.
...
Plus `rows={4}` on the textarea, 48px inputs, 16px text, and the 2px focus ring (C-cross-13).
...
- Conference-room on phones: `calc(100vw - 40px)`. On tablets: `min(34em, calc(100vw - 64px))`.
- Portrait: `128px` on phones and `240px` from 820.
...
| A-home-12 | P1 | Contact photo pushes the form 468px down | §6.1 row 07 (39:14 band, 125px) |
